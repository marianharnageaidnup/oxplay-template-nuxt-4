/**
 * Server-Side Production-Safe Logger
...
class ServerLogger {
  ...
  private sanitize(data: unknown): unknown {
    ...
    const sensitiveKeys = [
      ...
      'cookie',
      'csrf',
      'user', // Don't log full user objects
    ];
  ...
  private format(level: LogLevel, message: string, context?: LogContext): string {
    ...
    if (sanitizedContext && Object.keys(sanitizedContext).length > 0) {
      return `[${timestamp}] [SERVER] [${level.toUpperCase()}] ${message} ${JSON.stringify(sanitizedContext)}`;
    }

    return `[${timestamp}] [SERVER] [${level.toUpperCase()}] ${message}`;
  }
  ...
  /**
   * API error logging - specialized for external API errors
   */
...
// Export as a function that returns the logger (Nuxt auto-import friendly)
export function useLogger() {
  return loggerInstance;
}

// Also export the instance directly for direct imports
